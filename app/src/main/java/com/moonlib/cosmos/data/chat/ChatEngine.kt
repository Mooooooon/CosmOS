...
object ChatEngine {
    ...
    suspend fun getAiResponse(
        ...
        val responseText = if (isGeminiOfficial) {
            ...
            executeOpenAI(baseUrl, modelName, apiKey, temperature, systemPrompt, recentMessages, activeProfile.serviceType, contact.nickname)
        }
        ...
        try {
            ...
            for (i in recentMessages.indices) {
                ...
                val finalContent = if (i == recentMessages.lastIndex && msg.senderId == "user") {
                    msg.content + "\n(注意：你必须以指定的 JSON 格式输出回复，不要包含任何 markdown 块或废话)"
                } else {
                    msg.content
                }
                sbPrompt.append("$roleName: $finalContent\n")
            }
            sbPrompt.append("请记住你是谁，直接输出你作为角色的下一组符合 JSON 格式的回复：")

            val userInputText = recentMessages.lastOrNull { it.senderId == "user" }?.content ?: ""

            val logRepo = AiLogRepository(context)
            logRepo.saveLog(
                characterName = contact.nickname,
                modelName = modelName,
    ...
    private fun executeGeminiOfficial(
        ...
        // 组装 Gemini 内容 structure：将 System Prompt + 历史纪录合为一段提示词发送
    ...
    /**
     * 调用 OpenAI/DeepSeek 标准接口（带 JSON Mode / Structured Outputs 配置）
     */
    private fun executeOpenAI(
        ...
        // 2. 对话历史聚合装填，并在最后一条用户输入末尾强力注入约束指令，且合并连续助手回复防止模仿偏差
        var i = 0
