 ...
 * 聊天 AI 回复响应引擎
 *
 * 职责单一：负责组织对话历史上下文，调用当前激活的 AI 模型，获取并保存拟真的角色回复消息。
 */
object ChatEngine {

    /**
     * 玩家发送消息并异步获取 AI 的回复消息。
     * @param context Android 上下文
     * @param contact 当前聊天的联系人
     * @return AI 回复的 ChatMessage 对象
     */
    suspend fun getAiResponse(
        ...
        val systemPrompt = """
            ...
            3. 单次回复严禁长篇大论，字数应控制在 1 到 3 句话之内（建议不超过 50 字）。
            4. 充分使用契合角色性格的语气助词、标点符号，也可发送带括号的动作表情（如：[微笑]、[傲娇]、*眨眼* 或 Emoji）。
        """.trimIndent()
        ...
        chatRepo.saveMessage(contact.id, aiMessage)
    ...
    private fun executeGeminiOfficial(
        ...
        fullPromptBuilder.append("请记住你是谁，直接输出你作为角色的下一句聊天回复：")
